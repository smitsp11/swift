...
enum SparkFactory {
    ...
    ///
    /// - Parameters:
    ///   - start: Source neuron position.
    ///   - end: Destination neuron position.
    ///   - duration: Traversal time (based on pathway myelination).
    ///   - pathway: The neural pathway (used to scale visual intensity).
    ///   - anchor: Scene anchor to parent the spark to.
    ///   - cancellables: Combine cancellable storage for cleanup subscription.
    ///   - onArrival: Callback when spark reaches the destination.
    @MainActor
    ...
    static func fireCascade(
        ...
        for (index, info) in pathways.enumerated() {
            ...
            // Use timer-based delay to fire each spark with a stagger
